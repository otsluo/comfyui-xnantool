...
### 8. 视频尺寸预设节点 (VideoSizePresetNode)
- **位置**: `XnanTool/预设`
- **功能**: 提供视频常用的尺寸预设，包括1:1、3:4、4:3、9:16、16:9等多种比例，以及480p、720p、1080p、2K、4K、8K等多种分辨率
- **输入参数**:
  - `video_size_preset` (STRING): 视频尺寸预设选项，格式为"比例-分辨率-尺寸"或"正方形 1:1"
- **输出参数**:
  - `width` (INT): 视频宽度
  - `height` (INT): 视频高度
- **适用场景**: 当需要快速选择标准视频尺寸时使用，特别适用于视频生成和处理任务

### 9. 分辨率预设节点 (ResolutionPresetSelector)
- **位置**: `XnanTool/预设`
- **功能**: 提供常见分辨率名称的快速选择，包括1K、2K、4K和8K等标准分辨率名称
- **输入参数**:
  - `resolution_preset` (STRING): 分辨率预设选项，包括1K、2K、4K、8K等标准分辨率名称
- **输出参数**:
  - `resolution` (STRING): 选中的分辨率名称
- **适用场景**: 当需要快速选择标准分辨率名称时使用，特别适用于需要指定分辨率的场景