...
### 📤 预设图像上传节点-【新】 (PresetImageUploadNode)
- **位置**: `XnanTool/实用工具/预设`
- **功能**: 为提示词预设添加图像预览
- **输入**: 预设名称、图像
- **输出**: 状态信息(status_message)
...
<details>
<summary>🖼️ 批量加载图片节点使用</summary>

### 🖼️ 批量加载图片节点使用
1. 在节点菜单中找到 `XnanTool/图像处理` -> `批量加载图片`
2. 选择加载模式：
   - `加载全部`：加载指定目录中的所有图片（可设置最高加载数量）
   - `加载单张`：根据索引加载目录中的单张图片
3. 设置图片路径：
   - 可以是包含图片的目录路径
   - 也可以是单张图片的完整路径
4. 根据模式设置其他参数：
   - `加载全部`模式：设置`max_images`参数限制最多加载的图片数量
   - `加载单张`模式：设置`index`参数指定要加载的图片索引
5. 执行节点加载图片
6. 节点输出：
   - `images`：加载的图片张量（批量模式为(N,H,W,C)格式，单张模式为(1,H,W,C)格式）
   - `filenames`：加载的图片文件名列表
   - `count`：实际加载的图片数量
