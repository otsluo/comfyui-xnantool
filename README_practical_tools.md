...
### 📏 分辨率预设节点 (ResolutionPresetSelector)
- **位置**: `XnanTool/预设`
- **功能**: 提供常见分辨率名称的快速选择，包括1K、2K、4K和8K等标准分辨率名称
- **输入**:
  - `resolution_preset`: 分辨率预设选择
- **控件**:
  - `resolution_preset`: 分辨率预设（1K、2K、4K、8K等）
- **输出**:
  - `resolution`: 分辨率
- **适用场景**:
  - 快速选择常用分辨率名称
...
- **功能**: 将图像序列保存为视频文件
- **输入**: 
  - `images`: 图像序列 - 要转换为视频的图像序列
  - `filename_prefix`: 文件名前缀 - 输出视频文件名前缀
  - `fps`: 帧率 - 视频帧率（1-60 FPS）
  - `video_format`: 视频格式 - 输出视频格式（MP4/AVI/MOV/MKV）
  - `codec`: 编码器 - 视频编码器（mp4v/XVID/MJPG/X264/X265）
  - `audio`: 音频 - 可选的音频输入，用于与视频合并
  - `output_path`: 输出路径 - 视频保存路径（留空使用默认输出路径）
- **输出**:
  - `video_path`: 视频文件路径
- **适用场景**:
  - 将图像序列制作成视频
