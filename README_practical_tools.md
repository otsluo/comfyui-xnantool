...
### 📝 循环生成器节点 (LoopGeneratorNode)
- **位置**: `XnanTool/实用工具`
- **功能**: 支持最多3层嵌套循环，生成循环组合，每层使用多行字符串输入
- **输入**:
  - 第1层项目(layer1_items)：第1层项目列表（多行字符串）
  - 第2层项目(layer2_items)：第2层项目列表（多行字符串）
  - 第3层项目(layer3_items)：第3层项目列表（可选，多行字符串）
  - 分隔符(separator)：层与层之间的分隔符
  - 输出分隔(line_separator)：每行结果之间的分隔符（换行/逗号/分号/制表符）
- **输出**:
  - 结果：循环组合结果字符串
  - 总数：组合总数
- **适用场景**:
...
### 📊 计数器节点 (CounterNode)
- **位置**: `XnanTool/实用工具`
- **功能**: 每次运行时数值加一的计数器，支持多种计数模式
- **输入**:
  - 计数器ID(counter_id)：计数器的唯一标识符，不同ID独立计数
  - 起始值(start_value)：计数器的起始值
  - 增量(increment)：每次增加的数值
  - 重置(reset)：是否重置计数器
  - 上限值(max_value)：上限值（0表示不限制）
  - 超过上限重新计数(restart_on_max)：达到上限后是否重新计数
  - 超过上限中止运行(abort_on_max)：达到上限后是否中止运行
  - 触发器(trigger)：输入任意值触发计数
- **输出**:
  - 计数值：当前计数器的值
  - 信息：状态信息
- **适用场景**:
...
### 📁 列出文件节点 (ListFilesNode)
- **位置**: `XnanTool/实用工具`
- **功能**: 读取输入文件夹并输出该文件夹下的所有文件名称列表
- **输入**:
  - 输入文件夹路径(input_directory)：要读取的文件夹路径
  - 递归搜索(recursive)：是否递归搜索子文件夹
  - 分隔符(separator)：多个文件名称之间的分隔符
  - 文件扩展名过滤(file_extensions)：要过滤的文件扩展名（多个用逗号分隔）
- **输出**:
  - file_list：文件列表字符串
  - count：文件数量
- **适用场景**:
...
### 📂 列出文件夹节点 (ListFoldersNode)
- **位置**: `XnanTool/实用工具`
- **功能**: 读取输入文件夹并输出该文件夹下的所有文件夹名称列表
- **输入**:
  - 输入文件夹路径(input_directory)：要读取的文件夹路径
  - 递归搜索(recursive)：是否递归搜索子文件夹
  - 分隔符(separator)：多个文件夹名称之间的分隔符
- **输出**:
  - folder_list：文件夹列表字符串
  - count：文件夹数量
- **适用场景**:
  - 批量处理文件夹前的文件夹列表获取
  - 文件夹结构检查
...
### 📁 批量创建文件夹节点 (CreateFolderNode)
- **位置**: `XnanTool/实用工具`
- **功能**: 批量创建文件夹，支持多级目录结构
- **输入**:
  - 基础路径(base_path)：文件夹创建的基础目录路径
  - 文件夹路径(folder_names)：要创建的文件夹路径列表（每行一个，支持多级目录）
  - 已存在时(exist_ok)：如果目标文件夹已存在时的处理方式（true=忽略，false=报错）
- **输出**:
  - status：状态信息
  - info：详细信息
- **适用场景**:
...
### 🔀 循环生成器输出转接节点 (LoopGeneratorOutputSplitterNode)
- **位置**: `XnanTool/实用工具`
- **功能**: 将循环生成器的输出按索引选择某一行，然后拆分成多个输出端口
- **输入**:
  - 输入字符串(input_string)：循环生成器的输出结果（多行字符串）
  - 分隔符(separator)：用于拆分的分隔符
  - 行索引(line_index)：要提取的行索引（从0开始）
- **输出**:
  - 端口1：第1个值
  - 端口2：第2个值
  - 端口3：第3个值
- **适用场景**:
  - 将循环生成器的输出结果拆分成多个独立的值
  - 从循环生成的组合中提取特定行的各个部分
  - 与其他节点配合使用，实现更复杂的数据处理


### 📝 保存文本节点 (SaveTextNode)
- **位置**: `XnanTool/实用工具`
- **功能**: 将文本保存为指定格式的文件，支持txt、csv、md格式，支持多种文件存在模式
- **输入**:
  - 文本(text)：要保存的文本内容
  - 文件路径(file_path)：文件保存路径
  - 文件名(filename)：文件名
  - 扩展名(extension)：文件扩展名（txt/csv/md）
  - 文件存在时模式(exist_mode)：文件已存在时的处理方式（覆盖/直接追加/换行追加/跳过）
  - 文本前缀(text_prefix)：添加到文本前的前缀
- **输出**:
  - save_result：保存结果信息
- **适用场景**:
  - 保存提示词或其他文本内容
...
### 📹 保存视频节点 (SaveVideoNode)
- **位置**: `XnanTool/实用工具`
- **功能**: 将图像序列保存为视频文件
- **输入**: 
  - 图像序列(images)：要转换为视频的图像序列
  - 文件名前缀(filename_prefix)：输出视频文件名前缀
  - 帧率(fps)：视频帧率（1-60 FPS）
  - 视频格式(video_format)：输出视频格式（MP4/AVI/MOV/MKV）
  - 编码器(codec)：视频编码器（mp4v/XVID/MJPG/X264/X265）
  - 音频(audio)：可选的音频输入，用于与视频合并
  - 输出路径(output_path)：视频保存路径（留空使用默认输出路径）
- **输出**:
  - video_path：视频文件路径
- **适用场景**:
  - 将图像序列制作成视频
