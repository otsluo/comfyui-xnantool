    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "output_excel_path": ("STRING", {"default": "", "multiline": False}),
                "sheet_name": ("STRING", {"default": "Sheet1", "multiline": False}),
            }
        }

    RETURN_TYPES = ("STRING", "STRING")
    RETURN_NAMES = ("status", "excel_file_path")
    
    FUNCTION = "convert_md_to_excel"
    
    CATEGORY = "XnanTool/实用工具"

    def convert_md_to_excel(self, md_file_path, output_excel_path, sheet_name="Sheet1"):
        """
        ...
        try:
            ...
            status = f"成功将 {md_file_path} 转换为 {output_excel_path} (工作表: {sheet_name})"
            return (status, output_excel_path)
