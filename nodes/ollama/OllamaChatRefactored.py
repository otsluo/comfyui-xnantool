...
class OllamaChatRefactored:
    """
    重构版的 Ollama 聊天节点，具有以下增强功能：
    1. 支持连接验证和重连功能
    2. 异步处理支持
    3. 增强的错误处理和日志记录
    4. 改进的会话管理
        ...
        return {
            "required": {
                ...
                "think": (
                    ...
                    {
                        "default": False,
                        "tooltip": "如果启用，模型将在回答之前进行思考过程。这可以带来更准确的结果。然后可以将思考过程作为单独的输出用于调试或理解模型如何得出答案。某些模型不支持此功能。",
                    },
                ),
                "format": (
                    ["text", "json"],
                    {
                        "default": "text",
                        "tooltip": "响应的输出格式。'text'将返回纯文本响应，而'json'将返回JSON格式的结构化响应。当模型是较大管道的一部分并且需要对响应进行额外处理时，这很有用。",
                    },
                ),
                "validate_connection": (
                    "BOOLEAN",
            ...
            "optional": {
                ...
                "meta": (
                    ...
                    {
                        "forceInput": False,
                        "tooltip": "使用此输入链接多个'Ollama聊天'节点。在这种情况下，连接性和选项输入会被传递。",
                    },
                ),
                "history": (
                    "OLLAMA_HISTORY",
    ...
    async def ollama_chat_async(
        ...
        if "connectivity" not in meta or meta["connectivity"] is None:
            raise ValueError("'connectivity'必须存在于meta中。")
    ...
    def ollama_chat_refactored(
        ...
        if reconnect:
            try:
                ...
                error_msg = f"异步Ollama聊天请求失败: {str(e)}"
                print(error_msg)
                raise Exception(error_msg)
        else:
            # 同步处理
